...
def analyze_pcap(interactive, path, secrets):
    ...
    for packet in pcap_filtered:
        try:
            ...
            if "doh." in dns_query or "dns" in dns_query or "dot." in dns_query or "dns.google" in dns_query:
                    dns_query = ''
        except:
            pass
        try:
            server_name = str(packet.tls.handshake_extensions_server_name)
            if "doh." in server_name or "dns" in server_name or "dot." in server_name or "dns.google" in server_name:
                server_name = ''
            http_host = ''
        except:
            pass
...
def socket_write(lhost, lport):
    try:
        ...
        context.load_cert_chain(certfile='/home/router/Escritorio/public.pem', keyfile='/home/router/Escritorio/key.key')
...
def main():
    ...
    folder_exists = os.path.isdir('/var/log/DHD')
    if folder_exists:
        file_exists = os.path.exists('/var/log/DHD/alerts.log')

        if not file_exists:
            f = open("/var/log/DHD/alerts.log", "w")
            f.close()
    else:
        os.mkdir('/var/log/DHD')
        f = open("/var/log/DHD/alerts.log", "w")
        f.close()
    ...
    if INTERACTIVE:
        ...
        while (True):
            print("")
            print(Fore.BLUE + "/////////////////////////////////////////")
            print(Fore.BLUE + "Welcome to Domain Hiding Detector.")
            print(Fore.BLUE + "/////////////////////////////////////////")
            print("")
